...
| A.2.2 | **NCA Pre-Pre-Training** — Short NCA stage before main training to bootstrap attention induction circuits | HIGH | Implemented | MEDIUM | LOW | Full implementation: `scripts/nca_generate.py` (data gen), `scripts/base_train_nca.py` (training helpers), integrated into `base_train.py` and `speedrun.sh`. Transfer protocol: keep attention + MLP weights, reinit embeddings/scalars (matches paper). **2026-03-15 redesign:** Added START/END delimiter tokens per grid frame (38 tok/grid), min_grid ICL masking (first grid frame not trained on), vocab +2, gzip complexity band (tunable min/max). Speedrun uses 2000 rules × 100 epochs = ~3,100 NCA training steps in ~10 min. **Known deviation:** effective batch size 64 (8×8 GPUs) vs paper's 16 — fewer gradient updates per data seen. **8xH100 d24 tested (pre-redesign):** CORE 0.2542 in 95 min with NCA n=4/500 steps + 24K vocab (below threshold). Needs re-test with redesigned tokenization + 32K vocab. |
