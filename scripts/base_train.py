...
parser.add_argument("--xsa-start-layer", type=int, default=-1, help="XSA (Exclusive Self-Attention): -1=off, 0=all layers, N=layers [N,n_layer)")
...
while True:
    ...
    if args.eval_every > 0 and (last_step or step % args.eval_every == 0):
        ...
        if val_bpb < min_val_bpb:
            min_val_bpb = val_bpb
        # Append BPB to CSV for easy monitoring: tail -f ~/.cache/nanochat/bpb_log.csv
        if ddp_rank == 0:
